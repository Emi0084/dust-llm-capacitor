    ...
    if (teamId) {
      if (
        ...
        if (content.includes('DEVELOPMENT_TEAM = "";')) {
          ...
          content = content.replace(
            /PRODUCT_BUNDLE_IDENTIFIER = io\.t6x\.llmchat;/g,
            `PRODUCT_BUNDLE_IDENTIFIER = io.t6x.llmchat;\n\t\t\t\tDEVELOPMENT_TEAM = ${teamId};`,
          );
        }
      }
    }
...
async function main() {
  ...
  // 1.1 Find physical device or boot simulator
  let udid;
  ...
  try {
    if (connectedDevice) {
      ...
      isPhysical = true;
      pass("1.1 Physical Device ready", `${connectedDevice.name} (${udid})`);
    } else {
      udid = getBootedUDID();
      if (!udid) {
  ...
  if (isPhysical && fs.existsSync(srcHtmlPath)) {
    // For physical devices, we must patch index.html before xcodebuild so it gets signed into the app bundle
  ...
  try {
    if (isPhysical) {
      console.log(`  → Copying model to iPhone (${MODEL_FORMAT})...`);
      ...
      let sizeMB = 0;
      try {
        sizeMB = Math.round(fs.statSync(MODEL_PATH).size / 1024 / 1024);
      } catch (e) {}
      pass("1.5 Model in container", `${sizeMB} MB (${MODEL_FORMAT})`);
    } else {
    ...
    fail("1.5 Model in container", err.message?.slice(0, 200) || "failed");
    process.exit(1);
  }
  ...
  try {
    ...
    pass(
      "1.6 HTML patched",
      `${MODEL_FORMAT} → ${isPhysical ? "Documents" : modelSimPath.split("/").slice(-3).join("/")}`,
    );
  } catch (err) {
